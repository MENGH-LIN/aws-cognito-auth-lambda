...
public class GetNameAction extends AbstractAction {
  ...
  public String handle(String request, Context lambdaContext) throws BadRequestException {
    ...
    return "{ \"name\" : \"" + adminGetUserResult.getUserAttributes().stream()
        .filter(attribute -> attribute.getName().equals("name")).findFirst().get().getValue() + "\" }";
  }
}
